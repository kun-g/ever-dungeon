(function() {
  ...
  exports.dbScripts = {
    searchRival: "local board, name = ARGV[1], ARGV[2];\nlocal key = 'Leaderboard.'..board;\nlocal config = {\n    {base=0.95, delta=0.02, rand= ARGV[3]},\n    {base=0.85, delta=0.03, rand= ARGV[4]},\n    {base=0.50, delta=0.05, rand= ARGV[5]}\n  };\nlocal count = 3;\nlocal rank = redis.call('ZRANK', key, name);\n\nlocal rivalLst = {};\nif rank <= count then\n  local flatLst = {}\n  flatLst = redis.call('zrange', key, 0, rank-1, 'withscores');\n  count = count - rank;\n  local tmpList = redis.call('zrange', key, rank+1, rank+count, 'withscores');\n  for k, v in ipairs(tmpList) do\n    table.insert(flatLst, v)\n  end\n  for i = 1, table.getn(flatLst), 2 do\n    table.insert(rivalLst,{flatLst[i],flatLst[i+1]})\n  end\nelse\n  rank = rank - 1;\n  for i, c in ipairs(config) do\n    local from = math.floor(rank * (c.base-c.delta));\n    local to = math.floor(rank * (c.base+c.delta));\n    local index = from\n    if  to ~=  from then \n      index = index + c.rand%(to - from);\n    end\n    index = math.floor(index);\n    redis.log(redis.LOG_WARNING,'----'..tostring(i)..'----')\n    redis.log(redis.LOG_WARNING,index)\n    redis.log(redis.LOG_WARNING,c.rand)\n    redis.log(redis.LOG_WARNING,from)\n    redis.log(redis.LOG_WARNING,to)\n    rivalLst[i] = redis.call('zrange', key, index, index, 'withscores');\n  end\nend\n\nreturn rivalLst;",
