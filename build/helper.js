(function() {
  ...
  exports.dbScripts = {
    ...
    tryAddLeaderboardMember: "local board, name, value = ARGV[1], ARGV[2], ARGV[3]; \nlocal key = 'Leaderboard.'..board; \nlocal score = redis.call('ZSCORE', key, name)\nredis.log(redis.LOG_VERBOSE, value);\nredis.log(redis.LOG_DEBUG, score=='null');\nredis.log(redis.LOG_WARNING, score==nil);\nif score == false then\n  score = value;\n  redis.log(redis.LOG_DEBUG, value);\n  redis.log(redis.LOG_DEBUG, type(value));\n  if type(value) == 'string' then\n    value = redis.call('ZCARD', key)\n    return {1, value}\n  end\n  return {2, redis.call('ZADD', key, name, score)}\nend\nreturn {3, score}"
  };
